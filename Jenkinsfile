    ...
    stages {
        ...
        stage('Build Image') {
            steps {
                ...
                sh 'docker tag reactimage:latest techomaniac83/dev:latest'
            }
        }
        stage('Docker login') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'dockerhub_cred', passwordVariable: 'PASS', usernameVariable: 'USER')]) {
                    ...
                    sh 'docker push techomaniac83/dev:latest'
                }
            }
        }
        stage('Deploy') {
            steps {
                    sh 'docker run -itd --name insura -p 80:5000 techomaniac83/dev:latest'
                }
            }
       // stage('Deploy') {
           // steps {
